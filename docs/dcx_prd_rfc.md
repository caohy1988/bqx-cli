# DCX: An Agent-Native BigQuery CLI
...
`dcx` is a proposed agent-native CLI for BigQuery. It is designed for two
...
`dcx` fixes that gap. It starts as a focused BigQuery and agent-analytics CLI,
...
`dcx` will be built as a Rust CLI with three long-term domains:

- `dcx <resource> <method>` for BigQuery API access
- `dcx analytics <command>` for agent analytics workflows
- `dcx ca <command>` for Conversational Analytics
...
- `dcx analytics evaluate --evaluator=latency|error-rate --threshold ...`
- `dcx analytics get-trace --session-id ...`
...
- `jobs query` proves `dcx` is still a real BigQuery CLI
...
- dataset: provided by `--dataset-id` or `DCX_DATASET`
- table: `agent_events`
...
- `dcx jobs query --query "SELECT 1"` succeeds against a real project

### Milestone 3: Analytics Readiness

Implement `dcx analytics doctor`.
...
Implement `dcx analytics evaluate`.
...
Implement `dcx analytics get-trace --session-id`.
...
If `dcx` is framed as anti-MCP, the architecture discussion will get derailed.

Mitigation:

- position `dcx` as the best default interface for BigQuery developer and
  agent workflows
...
This preserves the original `dcx` vision while keeping the first release
small enough to finish.

## 12. Decision

Proceed with a demo-first `dcx` MVP that proves:
