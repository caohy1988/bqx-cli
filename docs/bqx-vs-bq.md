# BQX vs. BQ: Technical Comparison and Architectural Rationale

This document compares `bqx` with the standard `bq` CLI and explains why
...
| **Auth model** | Coupled to `gcloud auth` | 5-level priority: `BQX_TOKEN` > `BQX_CREDENTIALS_FILE` > OAuth > ADC > gcloud |
...
**`bqx` approach:**
```bash
$ bqx --project-id myproject --dataset-id analytics analytics doctor
```
```json
{
  "status": "healthy",
...
```bash
$ bqx analytics evaluate --evaluator latency --threshold 5000 --last "bogus"
...
export BQX_PROJECT=myproject
export BQX_DATASET=analytics
...
shapes are actual `bqx` data structures.
...
**The `bqx` way** — 1 command, structured verdict:
...
**Demo command:**
```bash
export BQX_PROJECT=<your-project> BQX_DATASET=<your-dataset>
bqx analytics doctor
bqx analytics doctor --format text
bqx analytics doctor --format table
```
...
bqx analytics evaluate --evaluator latency --threshold 999999 --last 1h --exit-code
echo "Exit code: $?"

# Fails (exit 1) — set a low threshold
bqx analytics evaluate --evaluator latency --threshold 1 --last 1h --exit-code
echo "Exit code: $?"

# Error rate evaluator
bqx analytics evaluate --evaluator error-rate --threshold 0.05 --last 1h --exit-code

# Human-readable output
bqx analytics evaluate --evaluator latency --threshold 5000 --last 1h --format text
```
...
**The `bqx` way** — evaluate finds the slow sessions, get-trace shows the detail:
...
bqx analytics get-trace --session-id <session-id-from-above>

# Table format for quick visual scan
bqx analytics get-trace --session-id <session-id> --format table

# Text format for compact terminal output
bqx analytics get-trace --session-id <session-id> --format text
```

### Bonus: Dry-Run Inspection

Agents (and humans) can preview exactly what API call `bqx` will make:

```bash
$ bqx --project-id demo-project jobs query \
...
programmatically. For example, the `bqx-analytics-evaluate` skill tells
...
constantly. `bqx` designs these as complementary commands sharing the same
...
| Phase 3 (complete) | Conversational Analytics (`bqx ca ask`), natural language → SQL for BigQuery, 26 skills | `bq` has no CA integration; requires a new command domain |
