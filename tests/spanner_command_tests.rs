...
#[test]
fn top_level_help_includes_spanner() {
    let output = run_dcx(&["--help"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("spanner"),
        "Help should list 'spanner' command"
    );
}

#[test]
fn spanner_help_shows_subcommands() {
    let output = run_dcx(&["spanner", "--help"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
...
#[test]
fn spanner_instances_help_shows_list() {
    let output = run_dcx(&["spanner", "instances", "--help"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("list"), "Should show list subcommand");
}

#[test]
fn spanner_databases_help_shows_list() {
    let output = run_dcx(&["spanner", "databases", "--help"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("list"), "Should show list subcommand");
}
...
#[test]
fn spanner_instances_list_requires_project_id() {
    let output = run_dcx(&["spanner", "instances", "list"]);
