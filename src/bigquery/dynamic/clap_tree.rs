...
/// Build a list of top-level `clap::Command` groups from generated commands.
/// Groups commands by resource (e.g. "datasets", "tables") and adds action
/// subcommands (e.g. "list", "get") under each group.
pub fn build_dynamic_commands(commands: &[GeneratedCommand]) -> Vec<clap::Command> {
    ...
    for group_name in group_names {
        ...
        for gen_cmd in sorted_cmds {
            ...
            for arg in &gen_cmd.args {
                ...
                if arg.required {
                    // datasetId and tableId for path params are required
        ...
        let group_about = format!("BigQuery {group_name} operations (generated from API)");
...
/// API parameter names that are provided by global CLI flags
/// and should not be generated as per-command args.
const GLOBAL_PARAM_NAMES: &[&str] = &["projectId", "datasetId"];

/// Extract matched argument values from clap ArgMatches into a map.
/// Keys are the original API parameter names (camelCase).
pub fn extract_args(matches: &ArgMatches, cmd: &GeneratedCommand) -> HashMap<String, String> {
...
mod tests {
    ...
    fn load_commands() -> Vec<GeneratedCommand> {
    ...
    fn builds_expected_groups() {
        ...
        assert_eq!(names, vec!["datasets", "jobs", "tables"]);
    }
    ...
    #[test]
    fn datasets_get_skips_dataset_id_arg() {
        // datasetId is provided by the global --dataset-id flag,
        // so it should NOT appear in the generated get subcommand.
        let cmds = load_commands();
        let dynamic = build_dynamic_commands(&cmds);
        let datasets = dynamic.iter().find(|c| c.get_name() == "datasets").unwrap();
        let get = datasets
            ...
            .unwrap();
        let arg_names: Vec<&str> = get.get_arguments().map(|a| a.get_id().as_str()).collect();
        assert!(
            !arg_names.contains(&"dataset-id"),
            "dataset-id should be handled by global flag"
        );
    }
    ...
    fn validate_required_catches_missing_table_id() {
        ...
        let mut args = HashMap::new();
        // Provide datasetId (global) but omit tableId
        args.insert("datasetId".to_string(), "my_dataset".to_string());
        let result = validate_required_params(&args, tbl_get);
