...
Do not use for raw SQL queries without event-type filtering — use dcx-query instead.
...
Use `dcx jobs query` to create views for common event types:
...
Use `dcx tables list` to discover views in a dataset:
...
- Use `dcx tables list` to discover existing views
...
- Views created via `dcx jobs query` are standard SQL views, not authorized views
