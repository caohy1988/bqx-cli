...
`dcx ca ask --profile <looker-profile>` just like BigQuery.
...
dcx ca ask --profile sales-looker.yaml "What were the top 10 orders last month?"

# Text format for interactive exploration
dcx ca ask --profile sales-looker.yaml --format text "Revenue by region"

# JSON format for scripting
dcx ca ask --profile sales-looker.yaml --format json "Average order value" | jq '.sql'
```
