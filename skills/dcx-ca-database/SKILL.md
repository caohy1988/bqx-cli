...
routing automatically — you use `dcx ca ask --profile <db-profile>` and
...
- **AlloyDB**: see `dcx-ca-alloydb`
- **Spanner**: see `dcx-ca-spanner`
...
dcx ca ask --profile ops-alloydb.yaml "top error categories last 24h"

# Text format
dcx ca ask --profile finance-spanner.yaml --format text "total payments by region"

# JSON for scripting
dcx ca ask --profile app-cloudsql.yaml --format json "active users today" | jq '.results'
```
...
- Use `dcx-ca-alloydb` for AlloyDB-specific setup and troubleshooting
- Use `dcx-ca-spanner` for Spanner-specific query patterns
