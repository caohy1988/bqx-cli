---
name: bqx-models
description: Use bqx to manage BigQuery ML models via the list, get commands. Generated from the BigQuery v2 Discovery API.
---
...
Lists all models in the specified dataset. Requires `--project-id` and `--dataset-id`.
...
Gets the specified model resource by model ID. Requires `--project-id`, `--dataset-id`, and `--model-id`.
...
| `--project-id` | Yes | GCP project ID (global flag) |
| `--dataset-id` | Yes | BigQuery dataset (global flag) |
| `--model-id` | Yes | Model ID of the requested model |
...
- Use `models list` to discover available models in a dataset
