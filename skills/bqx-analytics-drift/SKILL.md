...
Do not use for one-time evaluations — use bqx-analytics-evaluate instead.
...
period and a **current** period. Use `bqx analytics evaluate` to capture metrics
...
- Drift detection is a workflow pattern using `bqx analytics evaluate`, not a standalone command
- Comparison logic must be done outside bqx (e.g., in a script or CI pipeline)
