---
name: dcx-ca-alloydb
description: AlloyDB-specific Conversational Analytics setup, profile configuration, and operational troubleshooting patterns using dcx ca ask.
---
...
dcx ca ask --profile ops-alloydb.yaml "what are the largest tables by row count?"
dcx ca ask --profile ops-alloydb.yaml "show active connections"

# Business queries
dcx ca ask --profile ops-alloydb.yaml "total orders by region last 7 days"
dcx ca ask --profile ops-alloydb.yaml --format text "who are our top customers?"

# Debugging
dcx ca ask --profile ops-alloydb.yaml "show recent slow queries"
dcx ca ask --profile ops-alloydb.yaml "are there any locks or blocked queries?"
```
