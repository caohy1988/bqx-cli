---
name: bqx-ca-spanner
description: Spanner-specific Conversational Analytics setup, profile configuration, and business query patterns using bqx ca ask.
---
...
bqx ca ask --profile finance-spanner.yaml "who are the top 5 customers by spend?"
bqx ca ask --profile finance-spanner.yaml --format text "daily transaction count this week"

# Schema exploration
bqx ca ask --profile finance-spanner.yaml "show all tables"
bqx ca ask --profile finance-spanner.yaml "what columns does the orders table have?"

# Analytical queries
bqx ca ask --profile finance-spanner.yaml "average order value by month"
bqx ca ask --profile finance-spanner.yaml --format json "orders with amount over 1000" | jq '.results | length'
```
