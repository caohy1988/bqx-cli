...
### Using with dcx analytics views
...
- Include views for pre-filtered data access (e.g., per-event-type views from `dcx analytics views`)
- Add verified queries for frequently asked questions to improve accuracy
- Use `dcx ca add-verified-query` to add queries incrementally after creation
